...
const primaryMain: NavItemDef[] = [
  ...
  { href: "/fish", label: "nav.findFish", icon: Fish },
  { href: "/conditions", label: "nav.conditions", icon: CloudSun },
  { href: "/boards", label: "nav.biteReports", shortLabel: "nav.bites", icon: Radar },
  { href: "/gear", label: "nav.gear", icon: Backpack },
];

/** Inline at >= xl; folded into the "More" menu on narrower desktops. */
const primarySecondary = (fishId: boolean): NavItemDef[] => [
  ...(fishId ? [{ href: "/identify", label: "nav.identify" as MessageKey, icon: Camera }] : []),
  { href: "/trips", label: "nav.trips", icon: Route },
  { href: "/crews", label: "nav.crews", icon: UsersRound },
...
const personal: NavItemDef[] = [
  ...
  { href: "/search", label: "nav.findAnglers", icon: Search },
  { href: "/my-gear", label: "nav.myGear", icon: Box },
  { href: "/catches", label: "nav.myCatches", icon: Trophy },
  { href: "/spots", label: "nav.savedSpots", icon: MapPin },
  { href: "/settings", label: "nav.profileSettings", icon: Settings },
];
...
function DesktopNavLink({
  ...
  return (
    ...
    >
      ...
      {item.shortLabel ? (
        <>
          <span className="hidden min-[1400px]:inline">{t(lang, item.label)}</span>
          <span className="min-[1400px]:hidden">{t(lang, item.shortLabel)}</span>
        </>
      ) : (
        t(lang, item.label)
      )}
...
function MoreMenu({ items, pathname, lang }: { items: NavItemDef[]; pathname: string; lang: LanguageCode }) {
      ...
      {open && (
        ...
        >
          {items.map(({ href, label, icon: Icon }) => (
            ...
            >
              <Icon className="size-[18px] text-ink-500" /> {t(lang, label)}
            </Link>
          ))}
        </div>
      )}
...
export function TopNav({
  ...
  const lang: LanguageCode = user?.language ?? "en";
      ...
      <div className="mx-auto max-w-6xl px-4 h-20 flex items-center gap-4">
        ...
        <nav aria-label="Primary" className="hidden lg:flex items-center gap-1 ml-4">
          {primaryMain.map((item) => (
            <DesktopNavLink key={item.href} item={item} pathname={pathname} lang={lang} />
          ))}
          {secondary.map((item) => (
            <DesktopNavLink key={item.href} item={item} pathname={pathname} lang={lang} className="hidden min-[1360px]:flex" />
          ))}
          <MoreMenu items={secondary} pathname={pathname} lang={lang} />
        </nav>
              ...
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-60 rounded-xl bg-white shadow-lift border border-sand-200 py-2 animate-fade-up max-h-[calc(100dvh-6.5rem)] overflow-y-auto overscroll-contain">
                  ...
                  <div className="lg:hidden border-b border-sand-100 pb-1 mb-1">
                    <RegionSwitcher current={user.region} lang={lang} />
                  </div>
                  ...
                  <div className="lg:hidden border-b border-sand-100 pb-2 mb-2">
                    {primaryAll(fishId).map(({ href, label, icon: Icon }) => (
                      ...
                      >
                        <Icon className="size-4 text-ink-500" /> {t(lang, label)}
                      </Link>
                    ))}
                  </div>
                  {user.username && (
                    <Link
                      ...
                      className="flex items-center gap-2.5 px-4 py-2.5 text-sm font-semibold text-ink-900 hover:bg-sand-100"
                    >
                      <UserCircle2 className="size-4 text-ink-500" /> {t(lang, "nav.myProfile")}
                    </Link>
                  )}
...
export function MobileTabs({ user, fishId = false }: { user: NavUser; fishId?: boolean }) {
  ...
  const lang: LanguageCode = user?.language ?? "en";
  const tabs = [
    { href: "/home", label: t(lang, "nav.home"), icon: Home },
    { href: "/fish", label: t(lang, "nav.findFish"), icon: Fish },
    fishId
      ? { href: "/identify", label: t(lang, "nav.identify"), icon: Camera }
      : { href: "/catches", label: t(lang, "nav.catches"), icon: Trophy },
    { href: "/conditions", label: t(lang, "nav.conditions"), icon: CloudSun },
    user?.username
      ? { href: `/u/${user.username}`, label: t(lang, "nav.profile"), icon: UserCircle2 }
      : { href: "/login", label: t(lang, "nav.profile"), icon: UserCircle2 },
  ];
  return (
